...
def main():
    ...
    configs = [
        ...
        {
            "name": "Triple Layer (Deep)",
            "hypertensor_config": [
                {"size": 128, "order": 2, "activation": "gelu"},
                {"size": 96, "order": 3, "activation": "swish"},
                {"size": 64, "order": 2, "activation": "gelu"},
            ],
            "fc_layers": [32],
        },
        {
            "name": "Quad Layer (Very Deep)",
            "hypertensor_config": [
                {"size": 128, "order": 2, "activation": "gelu"},
                {"size": 96, "order": 3, "activation": "swish"},
                {"size": 64, "order": 2, "activation": "gelu"},
                {"size": 32, "order": 3, "activation": "swish"},
            ],
            "fc_layers": [16],
        },
    ]
    ...
    optimizer_configs = {
        "SGD": lambda params: torch.optim.SGD(params, lr=base_lr, momentum=0.9),
    ...
    for config in configs:
        for opt_name, opt_fn in optimizer_configs.items():
            run_name = f"{config['name']} - {opt_name}"
            print(f"\nTraining: {run_name}")
